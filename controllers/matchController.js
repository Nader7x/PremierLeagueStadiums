...
const getAllMatches = async (req, res) => {
  ...
  const result = await Match.find().skip(skip).limit(limit).lean();
...
const getLiveMatches = async (req, res) => {
  ...
  //     return res.send(cachedData);
  // }
  const result = await Match.find({ status: true, endState: false })
    .lean()
    .populate({
      path: "homeTeam",
      populate: {
...
const getHistoryMatches = async (req, res) => {
  ...
  const result = await Match.find({ endState: true })
    .lean()
    .populate({
      path: "homeTeam",
      populate: {
...
const goal = async (req, res) => {
  ...
  console.log(isHome);
  if (
    !homeTeam["squad"].includes(req.body.player) &&
    !awayTeam["squad"].includes(req.body.player)
  ) {
  ...
  if (
    match["cards"].get(req.body.player) &&
    match["cards"].get(req.body.player) === "red"
  ) {
  ...
  if (isHome) {
    await Match.findByIdAndUpdate(req.body.match, {
  ...
  if (match["goals"].get(req.body.player)) {
    ...
    result = await Match.findByIdAndUpdate(
      ...
      {
        goals: matchofGoals,
        $push: { events: [req.body.player, "goal"] },
      },
      { new: true },
    );
    // let message;
...
const endMatch = async (req, res) => {
  const result = await Match.findOneAndUpdate(
    { _id: req.params["id"], endState: false },
    { endState: true, status: true },
    { new: true },
  );
  if (!result) {
...
const giveCard = async (req, res) => {
  ...
  if (
    match["cards"].get(req.body.player) &&
    match["cards"].get(req.body.player) === "yellow"
  ) {
    const update = {};
    ...
    const result = await Match.findByIdAndUpdate(
      ...
      {
        $set: update,
        $push: { events: [req.body.player, "red"] },
      },
      { new: true },
    );
    res.send(result);
    ...
    const result = await Match.findByIdAndUpdate(
      ...
      {
        $set: update,
        $push: { events: [req.body.player, req.body.card] },
      },
      { new: true },
    );
    res.send(result);
...
const updateMatch = async (req, res) => {
  const result = await Match.findByIdAndUpdate(req.params["id"], req.body, {
    new: true,
  });
  res.send(result);
};
