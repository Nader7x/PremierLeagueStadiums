...
const getAllStadiumsWithTeam = async (req, res) => {
  const result = await Stadium.find().populate("homeTeam", "name").lean();
  res.send(result);
};

const getStadiumWithTeam = async (req, res) => {
  const result = await Stadium.findById(req.params["id"]).populate(
    "homeTeam",
    "name",
  );
  res.send(result);
...
const stadiumMatches = async (req, res) => {
  const result = await Match.find({ stadium: req.params["id"] })
